...
const CMVP_SEARCH_URL: &str =
    "https://csrc.nist.gov/projects/cryptographic-module-validation-program/validated-modules/search";
const CMVP_SEARCH_ALL_URL: &str =
...
#[derive(Parser, Debug)]
#[command(
    about = "Search NIST CMVP validated modules by algorithm. Missing CMVP terms fall back to scanning section 2.5 of cached security policy PDFs."
)]
struct Args {
    #[arg(required = true)]
...
fn main() -> Result<()> {
    ...
    let outcomes = search_products_for_terms(
        &client,
        &args.algorithms,
        ...
        match response {
            Ok(resp) => {
                ...
                if status.is_success() {
                    return Ok(resp.bytes().context("failed to read response body")?.to_vec());
                }
                if !matches!(
                    status,
...
fn cache_certificate_html(
    ...
    let path = cache_dir
        .join("certificates")
        .join(format!("{module_certificate}.html"));
    if path.exists() && !fresh {
        return fs::read_to_string(&path)
            .with_context(|| format!("failed to read {}", path.display()));
    }
    let html = fetch_text(client, certificate_url, &[])?;
    write_text(&path, &html)?;
    Ok(html)
}

fn cached_policy_paths(cache_dir: &Path, policy_url: &str) -> (PathBuf, PathBuf) {
    let filename = Path::new(policy_url)
        .file_name()
        .unwrap_or_default()
        .to_string_lossy()
        .into_owned();
    let pdf_path = cache_dir.join("security-policies").join(filename);
    let txt_path = pdf_path.with_extension("txt");
    (pdf_path, txt_path)
}

fn extract_pdf_text(client: &Client, cache_dir: &Path, policy_url: &str, fresh: bool) -> Result<String> {
    let (pdf_path, txt_path) = cached_policy_paths(cache_dir, policy_url);
    ...
    if !pdf_path.exists() || fresh {
        write_bytes(&pdf_path, &fetch_with_retries(client, policy_url, &[])?)?;
    }
...
fn resolve_cavp_algorithms(term: &str, cavp_algorithms: &[String]) -> Vec<String> {
    ...
    let mut matches: Vec<String> = cavp_algorithms
        .iter()
        .filter(|algorithm| !normalized_term.is_empty() && normalize_for_match(algorithm).contains(&normalized_term))
        .cloned()
        .collect();
    matches.sort();
    matches
...
fn normalize_cmvp_algorithm(term: &str, options: &HashMap<String, String>) -> Result<(String, String)> {
...
fn status_listing_url(status: &str) -> (&'static str, Vec<(&'static str, &'static str)>) {
    if status == "Active" {
        ...
        (
            CMVP_SEARCH_URL,
            vec![("SearchMode", "Advanced"), ("CertificateStatus", if status == "Historical" { "Historical" } else { "Revoked" })],
        )
    }
}
...
fn fetch_status_catalog(client: &Client, cache_dir: &Path, status: &str, offline: bool) -> Result<Vec<ResultRow>> {
    if offline {
...
fn search_cmvp_form(client: &Client, term: &str, options: &HashMap<String, String>, statuses: &[String]) -> Result<(String, Vec<ResultRow>, String)> {
...
fn policy_scan_supported(cache_dir: &Path) -> bool {
    cache_dir.join("security-policies").exists() || Command::new("pdftotext").arg("-v").output().is_ok()
}
...
fn extract_section_25(pdf_text: &str) -> Result<String> {
    ...
    let filtered = section
        ...
        .filter(|line| !matches!(line.as_str(), "Algorithm" | "CAVP" | "Cert" | "Properties" | "Reference"))
...
fn parse_section_matches(base_row: &ResultRow, section_text: &str, query_term: &str, candidate_algorithms: &[String]) -> Result<Vec<ResultRow>> {
...
fn parse_section_matches_offline(base_row: &ResultRow, section_text: &str, query_term: &str) -> Result<Vec<ResultRow>> {
...
fn scan_policy_for_algorithms(
    ...
    let certificate_html =
        cache_certificate_html(client, cache_dir, &base_row.certificate_url, &base_row.module_certificate, fresh)?;
...
fn scan_security_policies(
    ...
    if !policy_scan_supported(cache_dir) {
        let message = "pdftotext is not installed, so security-policy scanning is unavailable.".to_string();
        return Ok((
    ...
    let scanned: Vec<HashMap<String, Vec<ResultRow>>> = catalog
        .par_iter()
        .map(|row| scan_policy_for_algorithms(client, cache_dir, row, term_candidate_algorithms, fresh))
...
fn search_products_for_terms(
    ...
    for term in terms {
        ...
        if policy_scan
            && results_by_term.get(term).map(|outcome| outcome.rows.is_empty()).unwrap_or(false)
        {
    ...
    if !fallback_candidates.is_empty() {
        ...
        for current_status in &statuses {
            let (status_rows, status_messages) =
                scan_security_policies(client, cache_dir, current_status, &fallback_candidates, fresh, offline)?;
            for term in fallback_candidates.keys() {
                if let Some(rows) = status_rows.get(term) {
                    rows_by_term.entry(term.clone()).or_default().extend(rows.clone());
                }
...
fn render_table(outcome: &SearchOutcome) {
    ...
    let has_cavp_details = outcome.rows.iter().any(|row| !row.cavp_algorithm.is_empty());
    let header: Vec<&str>;
    ...
    println!("{}", format_row(&header.iter().map(|value| value.to_string()).collect::<Vec<_>>(), &widths));
    println!(
        "{}",
        widths
            .iter()
